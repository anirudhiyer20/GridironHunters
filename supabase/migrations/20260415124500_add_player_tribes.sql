...
add column if not exists tribe text not null default 'Unclaimed';
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'players_tribe_check'
  ) then
    alter table public.players
    add constraint players_tribe_check
    check (tribe in ('Combat', 'Forge', 'Storm', 'Tundra', 'Halo', 'Blaze', 'Shroud', 'Prowl', 'Unclaimed'));
  end if;
end $$;

create or replace function public.get_tribe_for_nfl_team(p_nfl_team text)
returns text
language sql
immutable
as $$
  select case upper(trim(coalesce(p_nfl_team, '')))
    when 'KC' then 'Combat'
    when 'CHIEFS' then 'Combat'
    when 'LV' then 'Combat'
    when 'LVR' then 'Combat'
    when 'OAK' then 'Combat'
    when 'RAIDERS' then 'Combat'
    when 'WAS' then 'Combat'
    when 'WSH' then 'Combat'
    when 'COMMANDERS' then 'Combat'
    when 'TEN' then 'Combat'
    when 'TITANS' then 'Combat'

    when 'PIT' then 'Forge'
    when 'STEELERS' then 'Forge'
    when 'IND' then 'Forge'
    when 'COLTS' then 'Forge'
    when 'SF' then 'Forge'
    when 'SFO' then 'Forge'
    when '49ERS' then 'Forge'
    when 'NYJ' then 'Forge'
    when 'JETS' then 'Forge'

    when 'ATL' then 'Storm'
    when 'FALCONS' then 'Storm'
    when 'SEA' then 'Storm'
    when 'SEAHAWKS' then 'Storm'
    when 'PHI' then 'Storm'
    when 'EAGLES' then 'Storm'
    when 'ARI' then 'Storm'
    when 'ARZ' then 'Storm'
    when 'CARDINALS' then 'Storm'

    when 'GB' then 'Tundra'
    when 'GBP' then 'Tundra'
    when 'PACKERS' then 'Tundra'
    when 'MIN' then 'Tundra'
    when 'VIKINGS' then 'Tundra'
    when 'BUF' then 'Tundra'
    when 'BILLS' then 'Tundra'
    when 'NE' then 'Tundra'
    when 'NEP' then 'Tundra'
    when 'PATRIOTS' then 'Tundra'

    when 'DAL' then 'Halo'
    when 'COWBOYS' then 'Halo'
    when 'LAR' then 'Halo'
    when 'LA' then 'Halo'
    when 'STL' then 'Halo'
    when 'RAMS' then 'Halo'
    when 'NO' then 'Halo'
    when 'NOS' then 'Halo'
    when 'SAINTS' then 'Halo'
    when 'LAC' then 'Halo'
    when 'SD' then 'Halo'
    when 'SDG' then 'Halo'
    when 'CHARGERS' then 'Halo'

    when 'CLE' then 'Blaze'
    when 'BROWNS' then 'Blaze'
    when 'DEN' then 'Blaze'
    when 'BRONCOS' then 'Blaze'
    when 'MIA' then 'Blaze'
    when 'DOLPHINS' then 'Blaze'
    when 'NYG' then 'Blaze'
    when 'GIANTS' then 'Blaze'

    when 'BAL' then 'Shroud'
    when 'RAVENS' then 'Shroud'
    when 'HOU' then 'Shroud'
    when 'HST' then 'Shroud'
    when 'TEXANS' then 'Shroud'
    when 'TB' then 'Shroud'
    when 'TBB' then 'Shroud'
    when 'BUCCANEERS' then 'Shroud'
    when 'CAR' then 'Shroud'
    when 'PANTHERS' then 'Shroud'

    when 'CIN' then 'Prowl'
    when 'BENGALS' then 'Prowl'
    when 'DET' then 'Prowl'
    when 'LIONS' then 'Prowl'
    when 'CHI' then 'Prowl'
    when 'BEARS' then 'Prowl'
    when 'JAX' then 'Prowl'
    when 'JAC' then 'Prowl'
    when 'JAGUARS' then 'Prowl'
...
set tribe = public.get_tribe_for_nfl_team(nfl_team);

create index if not exists players_tribe_idx
on public.players (tribe)
...
begin
  ...
  select
    ...
    public.get_tribe_for_nfl_team(coalesce(nullif(player_entry -> 'player' ->> 'maybeTeam', ''), 'FA')),
    null::integer,
...
grant execute on function public.get_tribe_for_nfl_team(text) to authenticated;
